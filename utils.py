"""From https://github.com/nbertagnolli/counsel-chat"""
...
def convert_df_to_conv_ai_dict(
    df: pd.DataFrame,
        ...
        for response_column in response_columns:
            ...
            if len(true_response) > 1:
                ...
                d = {
                    ...
                    "utterances": [
                        {"history": [question_combined], "candidates": candidates}
                    ],
                }
...
def sample_candidates(
    df: pd.DataFrame, current_id: Any, id_column: str, text_column: str, n: int
) -> List[str]:
    ...
    sampled_texts = (
        candidate_df[candidate_df[id_column] != current_id]
        .sample(n + 15)[text_column]
        .tolist()
    )
